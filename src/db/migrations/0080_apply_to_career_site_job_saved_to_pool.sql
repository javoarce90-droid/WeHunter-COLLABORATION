-- El candidato que se crea por auto-postulación (Career Site o su propio portal) nace
-- FUERA del pool de talento del recruiter (saved_to_pool = false, pisando el default `true`
-- de la columna, pensado para las altas manuales) — recién entra si un recruiter lo guarda
-- explícitamente ("Guardar en Talent Pool"). Si el candidato ya existía (enlace por email o
-- cuenta), no se le toca `saved_to_pool`: puede que ya lo hayan cargado a mano antes.
-- Mismo cuerpo que la 0077, solo cambia el insert de candidates.
