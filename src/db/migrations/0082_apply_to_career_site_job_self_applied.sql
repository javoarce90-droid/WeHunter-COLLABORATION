-- La postulación que crea el propio candidato (Career Site o su portal) queda marcada
-- self_applied = true. Es el único insert de applications que la setea en true: el alta
-- manual desde el pool (insertApplication, TS) usa el default `false` de la columna.
-- Mismo cuerpo que la 0080, solo cambia el insert de applications.
...
begin
  ...
  if v_candidate_id is null then
    ...
    values (
      ...
      v_profile.bio, v_profile.skills, 'portal', false
    )
...
-- Backfill: mejor proxy disponible para postulaciones que ya existían antes de esta columna.
-- De acá en adelante el dato es exacto (lo setea el insert de arriba).
update public.applications a
set self_applied = true
from public.candidates c
where c.id = a.candidate_id and c.source = 'portal';
